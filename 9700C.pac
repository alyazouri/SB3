...
// Cleaner / Improved Version
// Jordan Home IPv6 Lock + Stable Lobby + Stable Match
...
function isJordanIPv6(ip){
  ...
  // Special exact range rule for 2a01:9700
...
function FindProxyForURL(url, host){
  ...
  // If DNS fails, keep proxy path open instead of hard blocking
  if(!ip || ip === "")
    return PROXY;
  ...
  // Strict IPv6 lock: only allowed Jordan IPv6 passes
  if(isIPv6(fullIP) && !isAllowedPUBGIPv6(fullIP))
    return BLOCK;
  ...
  // only reset match lock when clear lobby traffic appears
  if(lobby && SESSION.active)
    resetMatchSession();

  // Lobby lock on first 3 hextets
  if(lobby){
  ...
  // Other PUBG traffic inside allowed Jordan IPv6 / allowed IPv4
  return PROXY;
}
